...
| B-03 寄存器 | `highreg` **0.308×**；regs/occ 三档相同，墙钟跟 local 128→1024 走 |
| B-09 布局 | `touch_fields=1` 时 SoA / AoS ≈ **13.6×** |
| C-06 Graph | 短核链 `stream/graph` ≈ **3.7～4.1×**；`work=4096` → **1.01×** |
| C-07 Persistent | 一任务一发 `launch/persistent` **32×→3740×**；`work=4096`→**314×**；oversub 更慢 |
| C-08 PDL | 同流依赖核；`serial/pdl` 贴 **1×**；定点 **1.13×**；`pdl_full`≈`pdl` |
...
| 重画实测图 | `python scripts/plot_b05_unified_memory.py` 等；C-01～C-08：`plot_c0N_*.py` |
...
**当前主线**：Module A ✅；Module B ✅；C-01～C-08 ✅。结构说明见 [`docs/仓库架构与现状.md`](docs/仓库架构与现状.md)。
...
./build/bin/03_compute_primitives_07_persistent --mode sweep
./build/bin/03_compute_primitives_07_persistent --mode sweep_work
./build/bin/03_compute_primitives_07_persistent --mode modes
./build/bin/03_compute_primitives_08_pdl --mode sweep
./build/bin/03_compute_primitives_08_pdl --mode sweep_tail
./build/bin/03_compute_primitives_08_pdl --mode modes
```
...
python scripts/plot_c08_pdl.py
```
...
| **Module C** | `article/03_compute_primitives` + `examples/03_compute_primitives` | 🟡 C-01～C-08 ✅；后半候选不阻塞 |
| **Module D–E** | 仅规划文档 | ⏳ |
