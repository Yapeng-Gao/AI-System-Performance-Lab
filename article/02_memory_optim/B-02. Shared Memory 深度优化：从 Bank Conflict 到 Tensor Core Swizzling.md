...
列访问怎么挤进一个 bank：`tile[32][32]` 上扫 `tile[tid][0]`，行距正好一圈 bank。

![图 1：列访问挤进同一个 Bank（8 格示意 32-way）](assets/B-02-fig1-column-bank.png)

> 图 1：`tile[tid][0]` 让同行距正好一圈 bank，多行同列打进 Bank 0。
...
![图 2：`[32][33]` 把同一列错开到不同 Bank](assets/B-02-fig2-padding.png)

> 图 2：多一列不参与计算，行跨度与 32 互质，列访问不再挤在一个 bank。
...
![图 3：`col ^ row` 打散映射，tile 仍是 `[32][32]`](assets/B-02-fig3-xor.png)

> 图 3：XOR 改的是物理列，不改容量。本章只证明能消列冲突，不是上 TMA 的仪式。
