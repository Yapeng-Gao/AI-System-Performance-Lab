...
| Device 内（本章） | `cp.async` / `cuda::pipeline` | GMEM → SMEM | CUDA event + intensity 曲线；NCU scoreboard |
...
1. **主证据**：`sweep` 的加速比曲线（写入 `docs/results/`）  
2. **旁证**：NCU 一组 `sync` vs `pipe2`（看 long scoreboard / mio_throttle / sm vs dram）  
...
> 跑完 `bash examples/02_memory_optim/07_profile_cp_async_pipeline.sh` 后，把 median / sweep 表贴到  
> `docs/results/B-07_cp_async_pipeline.md`，并同步更新本节。
...
```bash
ncu --set full -o cp_async_sync  ./bin/02_memory_optim_07_cp_async_pipeline --mode sync  --fma-iters 4
ncu --set full -o cp_async_pipe2 ./bin/02_memory_optim_07_cp_async_pipeline --mode pipe2 --fma-iters 4
```

关注：`smsp__warp_issue_stalled_long_scoreboard` 是否下降；是否出现 `mio_throttle`；不要只看单一 DRAM 吞吐。
