...
**配套可复现**：[Yapeng-Gao/AI-System-Performance-Lab](https://github.com/Yapeng-Gao/AI-System-Performance-Lab)（文章 + `.cu` + 实测表）。有用请 Star。 本章示例：[`examples/02_memory_optim/07_cp_async_pipeline.cu`](https://github.com/Yapeng-Gao/AI-System-Performance-Lab/blob/main/examples/02_memory_optim/07_cp_async_pipeline.cu)。
...
A-08 / B-06 讲的是 Host 侧流水线；本章关心的是 **kernel 内部**。两层 overlap **不要混谈**：

```text
B-06 Host：  CE [H2D][H2D]     SM   [K ][K ]      ← copy ∥ kernel（跨引擎）

B-07 Device（期望）：
  async copy:  [tile1][tile2][tile3]
  compute:         [c1 ][c2 ][c3 ]   ← 同一 CTA 内 copy ∥ compute

B-07 Device（现实：sync / async 立刻 wait）：
...
B-01 只测合并 / 对齐 / `float4`，§3.1 留了引擎地图。本章把 `cp.async` / pipeline 落成 **可复现 micro-bench + 决策表**。TMA 细节留给 B-08。

---

## 2. 物理模型：为什么要绕过寄存器

### 2.1–2.2 旧路径 LDG→RF→STS vs Ampere `cp.async` / LDGSTS

传统「先搬进 shared」占寄存器、挂长 scoreboard，也难把「下一 tile 的 load」藏进「当前 tile 的算」。  
Ampere（sm_80+）提供 GMEM→SMEM 硬件异步拷贝：数据可 **不进寄存器文件**，直接落入 shared。

```text
旧路径：  GMEM ──LDG──► Register File ──STS──► SMEM ──► 计算
                      （占 RF / 长 scoreboard）

新路径：  GMEM ──cp.async / LDGSTS──► SMEM（旁路 RF）──► 计算
```
...
双缓冲 / 多缓冲：算当前 stage 时，下一 stage 的 copy 已在飞。
...
| fma_iters | sync_ms | pipe2_ms | pipe4_ms | speedup_pipe2 | speedup_pipe4 |
|----------|---------|----------|----------|---------------|---------------|
| 1 | 0.0169 | 0.0132 | 0.0140 | **1.273×** | **1.209×** |
| 2 | 0.0163 | 0.0142 | 0.0131 | **1.146×** | **1.241×** |
| 4 | 0.0174 | 0.0132 | 0.0150 | **1.315×** | **1.158×** |
| 8 | 0.0193 | 0.0173 | 0.0172 | **1.119×** | **1.121×** |
...
![B-07 实测：speedup vs fma_iters（RTX 5090，median）](assets/B-07-speedup-vs-fma.png)

> 数据源：`docs/results/B-07_sweep.csv`；重画：`python scripts/plot_b07_cp_async.py`。

**固定 mode（`fma_iters=8`，同一天裸跑；与上表 `fma=8` 同行另测，数量级一致）**
...
![B-07 实测：固定 mode 相对 sync（fma_iters=8）](assets/B-07-mode-speedup-bars.png)

> 数据源：`docs/results/B-07_modes.csv`；同上脚本一并生成。

**怎么读**

1. **何时能藏延迟**：极低 AI（`fma=1~4`）`pipe2` 可达约 **1.15～1.31×**；`fma=8~32` 仍约 **+6%～+12%**。  
2. **何时变慢**：过 ~64 后加速比掉向 ≤1；`fma=256` 约 **0.96×**——算力/短依赖变重后协议变纯开销。  
3. **换指令 ≠ 流水线**：同强度下 `async1` 比 `sync` **慢 ~8%**。  
4. **别盲目加深 / 别无脑 shared pipeline**：多数点上 `pipe2` 不差于甚至优于 `pipe4`；`pipe2_blk` 差于 thread-local。  
...
**本机 Blackwell（sm_120）已验证**：`docs/sass/blackwell/07_cp_async_pipeline.sass` 中大量出现例如：

```text
LDGSTS.E [R15], desc[UR10][R12.64] ;
ARRIVES.LDGSTSBAR.64.TRANSCNT [UR18] ;
...
LDG.E.CONSTANT R3, desc[UR10][R2.64] ;
STS [R0], R3 ;
```

| 观察 | 含义 |
|---|---|
| 多处 `LDGSTS.E` + `ARRIVES.LDGSTSBAR` | async / pipeline 路径的 GMEM→SMEM 异步拷贝生效 |
| 同文件仍可见 `LDG` + `STS` | `sync` 核对照（经寄存器）仍在二进制里，属预期 |

> 编译 `pipe2_blk` 时若出现 `static_var_with_dynamic_init` 警告，示例已用 `#pragma nv_diag_suppress` 处理；不影响 SASS 结论。
...
# 更新 docs/results/B-07_*.csv 后重画正文图
...
1. **先证明你是 latency-bound**（或至少低 AI），再上 pipeline——本机极低 AI 可达约 **1.15～1.31×**；  
2. **赚的是 outstanding copy ∥ compute**，不是指令名字（`async1` 立刻 wait 反而更慢；SASS 见 `LDGSTS`）；  
3. **用 intensity 扫描判停**——高 AI 掉到 ≤1 是正常物理结果，不是实现一定写错。
