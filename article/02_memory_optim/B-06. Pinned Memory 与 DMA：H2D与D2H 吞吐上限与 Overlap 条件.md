...
**配套可复现**：[Yapeng-Gao/AI-System-Performance-Lab](https://github.com/Yapeng-Gao/AI-System-Performance-Lab)（文章 + `.cu` + 实测表）。有用请 Star。 本章示例：[`examples/02_memory_optim/06_pinned_dma.cu`](https://github.com/Yapeng-Gao/AI-System-Performance-Lab/blob/main/examples/02_memory_optim/06_pinned_dma.cu)。
...
三类典型翻车：**(1) 伪异步**（pageable staging）、**(2) 伪 overlap**（单流首尾相接）、**(3) 链路打不满**（双向远低于 2×）。
...
### 2.1–2.2 Pageable staging vs Pinned DMA

普通 `malloc` 内存可被操作系统换出。GPU DMA 需要稳定的物理地址，因此对 pageable 做 `cudaMemcpy(Async)` 时，驱动常见路径是：CPU 拷进驱动 pinned 窗口 → DMA 到 device → 大块则按窗口反复 stage。
...
Host pinned ──DMA(CE)──► HBM     （CPU 不参与中间拷贝）

路径 B — Pageable（必须 staging）

pageable ──CPU memcpy──► staging pinned ──DMA(CE)──► HBM
注意：cudaMemcpyAsync 返回，常只意味着 staging 完成，不是 DMA 完成
```
...
```text
serial（1 stream）
Stream0  [H2D0][K0][H2D1][K1]...     端到端 ≈ Σ(copy + kernel)
...
![B-06 实测：各 mode 有效带宽（RTX 5090）](assets/B-06-mode-gbs-bars.png)

> 数据源：`docs/results/B-06_modes.csv`；重画：`python scripts/plot_b06_pinned_dma.py`。
...
![B-06 实测：pinned / overlap / serial 端到端（iters=256）](assets/B-06-overlap-median-bars.png)

> 数据源：`docs/results/B-06_overlap.csv`；同上脚本一并生成。
...
Memcpy：Host pinned ─DMA─► HBM 再给 SM 读。Mapped Zero-Copy：SM 经 PCIe 直接读 Host，数据仍在 Host——单遍可接近 PCIe，多遍每遍再打 PCIe。
...
# 更新 docs/results/B-06_*.csv 后重画正文图
