...
**配套可复现**：[Yapeng-Gao/AI-System-Performance-Lab](https://github.com/Yapeng-Gao/AI-System-Performance-Lab)（文章 + `.cu` + 实测表）。有用请 Star。 本章示例：[`examples/02_memory_optim/08_tma_intro.cu`](https://github.com/Yapeng-Gao/AI-System-Performance-Lab/blob/main/examples/02_memory_optim/08_tma_intro.cu)。
...
2. **收益来自引擎搬运 ∥ compute（prefetch）**，不是「换 TMA 指令本身更快」。本机：立刻 wait 的 `bulk1d`/`tensor2d` 约 **0.86～1.05×**；`pipe2` 低 AI 约 **1.69×**。NCU（`fma=1`）：`pipe2` 吞吐 ~742 vs sync ~351 GB/s；`bulk1d` 的 `inst_executed` 反而比 sync 多 ~13%。
...
### 5.3 NCU 旁证（WarpState + Memory，`fma_iters=1`）

```bash
DO_NCU=1 bash examples/02_memory_optim/08_profile_tma.sh ncu-only
```

**不要**用 ncu 附着时程序自打印的 ms（会被 replay 放大到秒级）。下表来自 section / CLI metrics；明细见 [`docs/results/B-08_tma.md`](../../docs/results/B-08_tma.md)。

| mode | Mem Throughput | Max BW | cycles/issued | 主导 stall | 一句话 |
|------|----------------|--------|---------------|------------|--------|
| `sync` | 351 GB/s | 19.9% | 14.1 | L1TEX scoreboard ~53% | 等全局加载 |
| `bulk1d` | 338 GB/s | 19.2% | 12.1 | branch ~30% | 立刻 wait，吞吐≈sync |
| `pipe2` | **742 GB/s** | **42%** | **8.4** | fixed latency ~34% | overlap 后短依赖抬头 |

指令量（同 512 warps）：`sync` **4.12M** vs `bulk1d` **4.67M**（**+13%**）——本 4 KiB MVP **没有**「换 TMA 就少指令」；barrier/elect 协议税更明显。大 tile / warp-spec 才是卸发令的主战场。

与裸跑一致：`pipe2` 才把故事从「等 L1TEX」推进到「短依赖」；`bulk1d` 立刻 wait 只是换 stall 类型。

### 5.4 SASS 旁证（确认走了 TMA / elect / mbarrier）

```bash
bash examples/02_memory_optim/08_dump_sass.sh
```

本机（`docs/sass/hopper|blackwell/08_tma_intro.sass`）要点：

| 观察 | 含义 |
|---|---|
| **`UTMALDG.2D`** | tensor-map 路径的 TMA 2D load 已进二进制 |
| **`ELECT` + `EIATTR_MBARRIER_*`** | 选举 issue 线程 + G2S mbarrier 完成模型生效 |
| `kernel_bulk1d` 仍见 **`LDG.E` + `STS`** | 1D 立刻 wait 反汇编不「纯净」；与 NCU 吞吐≈sync、event ≈1× 一致 |
| `kernel_sync` | 协作 load：LDG+STS 为主（预期） |

SASS 只证明**路径存在**；加速比仍以裸跑 `sweep` 为准。
...
- NCU：`pipe2` 主导 stall 已变短依赖、Mem Throughput 明显高于 sync → 与 event 结论互证；`bulk1d` 吞吐≈sync 则别为 TMA 而 TMA
