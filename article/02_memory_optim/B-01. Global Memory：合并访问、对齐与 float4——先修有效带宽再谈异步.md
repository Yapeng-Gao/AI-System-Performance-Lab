...
![图 1：合并落进约 4 个 32B sector；跨步打散成很多 sector](assets/B-01-fig1-coalesce.png)

> 图 1：warp 请求落进越少 sector 越好。大跨步才会「sectors 爆炸」；默认 `offset=1` 仍大体连续。
...
![图 2：同样 16B，四条 LDG.32 对一条 LDG.128](assets/B-01-fig2-float4.png)

> 图 2：`float4` 省的是发令槽。已贴带宽天花板时墙钟可以 ≈1×（本机 1.038×）。
