![B-05 封面：Unified Memory Fault / Prefetch](assets/B-05-unified-memory-cover.png)
...
**配套可复现**：[Yapeng-Gao/AI-System-Performance-Lab](https://github.com/Yapeng-Gao/AI-System-Performance-Lab)（文章 + `.cu` + 实测表）。有用请 Star。 本章示例：[`examples/02_memory_optim/05_unified_memory_pf.cu`](https://github.com/Yapeng-Gao/AI-System-Performance-Lab/blob/main/examples/02_memory_optim/05_unified_memory_pf.cu)。
...
但“Page Fault + 页面迁移”背后是一条很长的延迟链，远非一次简单的 DMA 拷贝。
...
```text
SM stall → UVM driver（CPU）→ 建迁移 → DMA（PCIe/NVLink）→ 页表/TLB → Warp 恢复
         └────────────── 延迟 + 序列化（不是纯带宽问题）──────────────┘
```
...
```text
Fault     ：访问时才搬（关键路径上 stall）
Prefetch  ：提前搬（独立 stream，再开 kernel）
Advise    ：只给 hint（PreferredLocation / AccessedBy / ReadMostly），不保证生效
```
...
# 或单组
nsys profile --trace=cuda,osrt,nvtx -o um_fault_trace --force-overwrite=true \
  ./build/bin/02_memory_optim_05_unified_memory_pf --mode fault --n 16777216 --iters 32 --runs 5 --warmup 1
```
...
4. 在 B 基础上添加与访问模式匹配的 `cudaMemAdvise`（本示例为 `PreferredLocation` + `AccessedBy`；**可写数据勿用 ReadMostly**），跑 C（同参数）5 轮，记录指标。
...
![B-05 实测：暖身后续跑 fault / prefetch / advise](assets/B-05-mode-latency-bars.png)

> 数据源：`docs/results/B-05_modes_warm.csv`；重画：`python scripts/plot_b05_unified_memory.py`。
...
| mean   | 9.8            | 被冷启动严重拉高，**不能**用均值评判 UM 稳态性能 |

![B-05 实测：冷启动 first vs 稳态 median（log 轴）](assets/B-05-cold-vs-warm.png)

> 数据源：`docs/results/B-05_cold_fault.csv`；同上脚本一并生成。

排序后三轮约为 `[0.23, 0.23, 29]` → **首轮 / 稳态中位数 ≈ 123×**，远超 §4.4 的判停阈值 1.5。在 `um_fault_trace.nsys-rep` 的 **CUDA HW** 行查看首轮 kernel 是否与 **UVM page fault / migration** 时间簇重叠。
...
转向显式管理之后，下一道门槛是：**Host 缓冲区是否允许 DMA 直达，以及 Async / 多 Stream 何时才是真 overlap**。

→ **B-06. Pinned Memory 与 DMA：H2D/D2H 吞吐上限与 Overlap 条件**（同目录 `B-06*.md`）
