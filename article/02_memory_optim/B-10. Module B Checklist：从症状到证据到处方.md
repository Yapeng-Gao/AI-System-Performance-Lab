...
| **对上处方** | §4：症状 → 证据 → 处方 → 回链 |
| **防混层** | §4：三层 Async + 布局 vs 引擎 |
| **索引** | §5 + `docs/results/B-10_checklist.md` |
...
| 时间线主要看见 | 症状层倾向 | 下一步 |
|---|---|---|
| H2D/D2H 很长，kernel 很短；或 memcpy 与 compute 首尾相接无重叠 | Host↔Device | §3 Host 行 → B-06 |
| Kernel 期间大量 page migrate / fault；首轮远慢于稳态 | UM | §3 UM 行 → B-05 |
...
只在 **kernel 已是主因** 时用。看 Speed of Light（见 §10-2）：
...
|---|---|---|---|
| B-01 | `02_memory_optim_01_global_mem_bandwidth` | 章内 mode | 正文 |
| B-02 | `02_memory_optim_02_shared_mem_bank_conflict` | 章内 mode | 正文 |
