...
2. **该换 SoA**：kernel **按字段**扫大批量记录、且每次只用宽 struct 中少量字段（粒子 / 顶点属性 / instance 状态）。文献可见数倍～数十倍差——**以本机 `sweep` 为准**，勿背博客绝对数。
3. **别盲目全量永久 SoA**：热路径几乎总整条记录读写、或宿主/API 强绑 AoS 时，先估转换成本；触达字段↑时 SoA 红利**通常会收窄**（AoS 同 sector 内多字段被 L2/扇区复用摊销），但不保证墙钟加速比回到 ~1。
4. **Transpose 是布局变换算子**：naive 跨步写 ≈ 带宽地板；处方是 **SMEM tile 重排**（GMEM 读合写合），`tile[T][T+1]` 消 bank 冲突（细节见 B-02），目标逼近 **copy** 带宽。
5. **判停**：主看 CUDA event **useful-payload GB/s** 与 SoA/AoS、tiled/naive 加速比；有 NCU 再看 `sectors/request`（理想 float 合并约 **4**；明显偏高先修布局）。

> 实测数字：跑通后回填 `docs/results/B-09_*`。主命令见 §5。
...
平台与完整表见 [`docs/results/B-09_layout.md`](../../docs/results/B-09_layout.md)。  
有 CSV 后重画：
...
实测图（有 CSV 后生成）：`assets/B-09-speedup-vs-touch.png`、`assets/B-09-transpose-gbps-bars.png`。

**怎么读（预期形状，待本机数字替换）**

1. `touch_fields=1`：SoA 应明显快于 AoS（数倍级常见；绝对倍数看本机）。  
2. `touch_fields→8`：加速比**通常收窄**（sector/L2 复用摊销），不要求回到 ~1×。  
3. `transpose_tiled`/`pad` ≫ `naive`，并靠近 `copy`；pad 相对 tiled 的增量通常小于「naive→tiled」那一跳。  
4. 程序在 transpose 计时前会做一次轻量正确性检查；失败直接报错退出（避免「错得很快」）。
