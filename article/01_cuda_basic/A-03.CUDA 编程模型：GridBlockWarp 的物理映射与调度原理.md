...
A-01 已经否定 1:1 接线。本章把四行写死，后面只在这张表上长细节。
...
GTE 是芯片级分发器，不属于任何一颗 SM。它做三件事：接 launch、维持还没驻留的 Block、在 SM **还有容量** 时把 Block 交出去。

![图 2：GTE 按容量派 Block，不跑 Warp](assets/A-03-fig2-gte.png)

> 图 2：空槽才领下一张工单。工程模型是「有容量才派」；硬件内部是 push 还是 pull，**不是语义保证**。

它**不会**：看某个 Block 还要跑多久、做访存预测、在 SM 之间搬已经驻留的 Block、按 `blockIdx` 保证执行顺序。
...
Block 一旦进某颗 SM：不迁移、不抢占、跑完才释放资源。Kernel 结束时间 = 最慢那个 Block。

Occupancy 描述的是：**这颗 SM 此刻能同时驻留多少 Warp / Block**，被四类上限卡住——寄存器、SMEM、每 SM 最大 Block 数、每 SM 最大 Warp 数。任一撞墙，GTE 再有工单也派不进去。
...
一波（wave）≈ 全卡当前能同时驻留的 Block 数。Grid 不是这个数的整数倍时，最后一波可能只剩几个 Block，其余 SM 空转。这就是尾波。
