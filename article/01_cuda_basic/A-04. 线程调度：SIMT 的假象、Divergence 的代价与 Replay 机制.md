...
32 条 lane 对条件的答案不一致，就是 **Warp divergence**。硬件不会让 A、B 同时铺满 32 lane——取指只有一份。它先让偶数 lane 跑 A（奇数 mask 掉），再让奇数跑 B（偶数 mask 掉），汇合之后 mask 恢复。两条路径的延迟**相加**。灰色 lane 仍占着 Warp 的发射槽，只是不写结果：Scheduler 不能在这一拍把槽让给另一个程序。
...
「约 2×」只在两条路径算力接近、没有被别的 Warp 藏住时像教科书。别的 Warp 就绪时，Scheduler 会穿插，墙钟上的比值会被藏掉一块。本机以示例打印的 `clock64` median 比为准，**不要把 2.0× 写进结论当实测**。

别在 Hello 里上全套 branchless。热路径上：能让同一 Warp 走同一支就走同一支（按 Warp 对齐的数据更好）；短条件交给编译器；长的、不可对齐的分叉，先量再改。手写 warp 原语去 C-01。
...
Volta 起 Independent Thread Scheduling：Warp 内每条线程有自己的 PC，汇合点显式化。等锁的人不必把已经走完分支的人冻死；调度器可以在不同 PC 的子集之间切换前进。Ada（RTX 40）也有 ITS，因为它从 Volta 继承，**和 Hopper 无关**。
...
32-way bank：教科书写 32×；流水线和其它 Warp 会藏掉一块，本机常见远小于 32。怎么 padding / swizzle 去 **B-02**，本章只证明「冲突会重放」。
