...
> **已交付**：Module A、B 收束；C-01～C-06 有 RTX 5090 / `sm_120` 实测。  
...
| Module A / B 收束；C-01～C-06 | Module D 算子、Module E 框架集成（规划） |
...
```text
A  物理映射 / 工具 / Roofline     →  建立「代码落在哪块硅」
B  访存 / 异步拷贝 / TMA / 布局   →  多数 kernel 先撞这里；出口 B-10
C  warp / CG / atomic / sync / fuse / graph
                                  →  协作与 launch 税；算子正文不在这
D  Reduce → Softmax/LN → GEMM     →  规划
E  扩展 / profiler / 部署         →  规划
```
...
| 章 | 主题 |
|---|---|
| B-01 | Global Memory：合并访问 → TMA 视角 |
| B-02 | Shared Memory：Bank / Padding / Swizzle |
| B-03 | 寄存器 spilling 与 occupancy |
| B-04 | L2 驻留 |
