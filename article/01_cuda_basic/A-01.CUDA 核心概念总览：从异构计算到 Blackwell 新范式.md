...
CPU 把晶体管花在控制、乱序、大缓存、分支预测上，单次响应要快：缓存没命中就去内存，也尽量只让这一条请求等。GPU 把面积花在重复的计算通道上，一次要喂饱成千上万条线程。某条指令在等 HBM 时，调度器换另一组就绪的 Warp 上去。藏延迟靠的是**并发足够多**，不是把单线程做得像 CPU。

所以不是「GPU 是更强的 CPU」。单线程、强分支、指针追逐，CPU 通常更合适；规则重复、数据并行，才轮到 Device。Host 仍然不可省：它发工单、搬数据、和操作系统说话。GPU 没有 syscall，也不能替你 `fopen`。

![图 1：CPU 延迟导向 vs GPU 吞吐导向](assets/A-01-fig1-cpu-gpu-die.png)

> 图 1：左边控制/缓存占比大，右边计算通道铺满。类比到此为止，不要从面积比去推 Occupancy。

工程含义：kernel 写得再满，Host 每次提交都排队、或数据还在 PCIe 上，端到端照样慢。热路径上的墙，经常是 launch 税或 H2D/D2H，不是「算力没用完」。C-06 测 launch 税；B-06 测 H2D。本章只把分工说清。

---

## 3. 逻辑层级不是硬件 1:1

CUDA 给你 Grid → Block → Thread，是为了写得动：你按数据切工单，不必知道这张卡有几颗 SM。硅片上的对应是**多对一 + 驻留**，不是三根电线一对一接上。

读成 1:1 接线，后面三章都会歪：occupancy 变成「每 SM 一个 Block」、divergence 变成「32 颗小 CPU」、TMA 变成「线程自己 memcpy」。A-03 把派发和 occupancy 写完；本章先把三张图的读法钉死。
...
图 4 没画出的一句：Thread 是 lane 上的寄存器状态，不是绑死一颗 Core。Block 占着该 SM 的 SMEM/寄存器直到结束。调度去 A-03。
...
`.cu` 里同时有 Host 的 `main` 和 Device 的 `__global__`。CPU 按 C++ ABI 编；GPU 按 SIMT 编，没有 syscall。没有哪个编译器能按一种语义吃完整份文件，所以 `nvcc` 是**驱动器**：拆开、分别请人干活、再缝回同一个可执行文件。

Device 侧还要冻两次。Frontend 先吐 **PTX**（虚拟 ISA，相对架构无关）；`ptxas` 或驱动 JIT 再冻成 **SASS**（绑死 `sm_XX`）。GPU 只执行 SASS。一份二进制里常装好几份 SASS，外加一份 PTX 给更新的卡现场编——这就是 Fatbin。为什么要虚/实两套架构、第一次启动为什么会 JIT，去 **A-06**。
...
`CMAKE_CUDA_ARCHITECTURES=native` 探测失败时写死：Ada 常见 `89`，5090 常见 `120`（代号见 §5）。想确认 Fatbin 里到底有哪些 `.text._Z…` 段，用示例旁的 `01_fatbin_inspect.sh`（`cuobjdump`）。
...
写下 `hello_kernel<<<1, 32>>>(data)` 时，Host **立刻返回**。这不是语法糖偷懒，是吞吐机器的默认合同：CPU 不该坐着等 GPU 把这一票算完，后面往往还有下一次 launch、下一次 H2D。GPU 什么时候真正开跑，由 Stream 上的队列决定。
...
1. 参数进 launch 包（常见路径会落到常量内存侧；ABI 细节 A-05）。
...
把 Host `chrono` 包住一次 `<<<>>>`，量到的是「提交 + 排队 + 执行 + 同步」，不是 kernel。计时用 warmup + CUDA event median，见 A-08 与 B-06 之后各章。
