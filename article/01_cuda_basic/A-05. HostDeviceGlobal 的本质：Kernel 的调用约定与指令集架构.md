...
写下 `foo<<<grid, block>>>(n, ptr)` 时，Host 不会按 C 的 calling convention 把东西推进 GPU 栈。Runtime 组一份 launch 描述：grid/block、stream、**参数字节**。这份字节进 **parameter buffer**。
...
**SASS** 绑具体架构：寄存器编号、管道、访存空间、barrier 都在这一层写死。一条 SASS 的语义是 **32 条 lane 锁步**（mask 掉的 lane 仍占发射槽，见 A-04）。固定字长，`cuobjdump -sass` 看到的就是它。
...
**PTX 看不出性能。** PTX 里的寄存器是虚拟的，指令条数和最终 SASS 也对不齐；`ptxas` 会重排、折叠、改成架构专有指令。PTX 决定「能编成什么」，SASS 决定「这张卡上到底发出去什么」。

Predicate mask、Replay、ITS 的前进语义都发生在 SASS / 硬件计数器上，**机制正文在 A-04**。本章不重开 Stall 表。Nsight 里的 stall 理由、replay 计数，用 A-04 的三列去读。
