...
- 常驻网格占满 SM 时，同设备上别的核会饿。生产里有时只占一部分 SM；本章不测多核共存（C-08 / A-08 也不在这里重开）。
...
4. 同流依赖核提前启动：见 **C-08 PDL**。named barrier 仍是候选 C-09，索引收束走 C-10。
...
下一章 **C-08 PDL**：同流依赖核提前启动，不重测本章一任务一发。C-09 仍候选。
