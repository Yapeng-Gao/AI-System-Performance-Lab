...
0. **全景主线已锁定，后续改动不得离线。** 先读 `docs/CUDA专栏规划.md` §2 / §2.2 / §3 / §4。A→E 能力链、各 Module 教法弧、展开权表是写稿坐标系，不是参考意见。禁止重排 Module、把展开权挪到别章、用邻章主题当本章开篇主线、把候选槽写成必须写的下一章。B 弧：形状 → on-chip → 跨空间 → 引擎 → 布局；B-09 与 B-01 是一层，现序迁就已发布。**只有**用户明确说「优化全景 / 改全景图 / 重锁 L0」才许改 §2 ASCII、展开权或教法弧；画像素全景图也以该 ASCII 为构图，禁止另起一套分层。局部润色、补数、修 `.cu` 同样受这条约束。
...
- `docs/CUDA专栏规划.md`（§2 全景 / §4 该 Module / §5 模板 / §7 当前焦点）+ 相邻章大纲
- 上一章正文「下一章钩子」与边界表
- 同 module 最近一章 `0N_*.cu`（CLI、计时、`--mode`）
- `docs/仓库架构与现状.md`

弄清：**本章在 §4 弧上的哪一格 / 深化什么 / 严禁重复什么（§2.2 展开权） / 钩子给谁 / 硬件门槛**。若改动会移动展开权或改教法弧 → **停手**，除非用户在优化全景。
...
- **TL;DR 带本机数字**
- **机制节写够。** TL;DR / 边界表是索引，不是正文。本章要建立的概念必须展开：是什么、为什么、在硬件/工具链上怎么发生、常见错读。读者读完应能转述。
- **「去 A-xx / B-xx」只用于邻章已有完整正文的课题。** 禁止用出口句替代本章该讲的概念课。
- 砍套话、重复小标题、抢邻章的课；**不砍科普性讲解。** 工程索引 ≠ 每章 150 行表+图注。Module A 概念章以讲清为准。
- 原理：ASCII 或原理图辅助讲解；图注中文；图用本地 `assets/`，禁止 `csdnimg`
- 决策表 / SOP / 误区 / 钩子
- §7 与 §10 **去重**（硬约束 8）
...
| 文件 | 更新什么 |
|---|---|
| `docs/CUDA专栏规划.md` | 该章状态 ✅/🟡 + 大纲链接；§7「当前焦点」只一条主线 |
...
| 边界清晰 | 不重讲上章；不做的在大纲/边界写明；未抢 §2.2 展开权、未改教法弧 |
...
- **未说「优化全景」却改 L0 / 教法弧 / 展开权**；或写稿时另起一套分层叙事
- **B-01/B-02 开篇主讲 TMA / async**；把 swizzle 写成「为 TMA 服务的下一站」
- 把 C-07～C-09 / D-07～D-09 候选槽写成必须交付的「下一焦点」
- 未确认大纲就写长文；大纲跳过文献调研
