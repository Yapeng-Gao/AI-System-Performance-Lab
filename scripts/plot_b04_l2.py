#!/usr/bin/env python3
"""Plot B-04 L2 residency results.

Reads:
  docs/results/B-04_modes.csv

Writes:
  article/02_memory_optim/assets/B-04-mode-ms-bars.png
  article/02_memory_optim/assets/B-04-speedup-vs-mixed.png
"""
...
MODES_CSV = ROOT / "docs" / "results" / "B-04_modes.csv"
...
MODE_ORDER = ["streaming", "hot", "mixed", "persist", "thrash"]
MODE_COLOR = {
    "streaming": MUTED,
    "hot": GREEN,
    "mixed": CYAN,
                ...
                rows.append(
                    {
                        ...
                        "speedup_vs_mixed": float(row["speedup_vs_mixed"]),
                    }
                )
...
def main() -> int:
    ...
    if not rows:
        print(f"no data rows in {MODES_CSV} (5090 numbers pending)", file=sys.stderr)
        return 2

    labels = [str(r["mode"]) for r in rows]
    ms = [float(r["median_ms"]) for r in rows]
    sp = [float(r["speedup_vs_mixed"]) for r in rows]
    ...
    ax.set_title("B-04: L2 residency modes (median)")
    for i, v in enumerate(ms):
        ax.text(i, v, f"{v:.3f}", ha="center", va="bottom", color=FG, fontsize=9)
    fig.tight_layout()
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out1 = OUT_DIR / "B-04-mode-ms-bars.png"
    ...
    ax.set_ylabel("speedup (mixed_ms / mode_ms)")
    ax.set_title("B-04: speedup vs mixed")
    for i, v in enumerate(sp):
        ax.text(i, v, f"{v:.2f}x", ha="center", va="bottom", color=FG, fontsize=9)
    fig.tight_layout()
    out2 = OUT_DIR / "B-04-speedup-vs-mixed.png"
