#!/usr/bin/env python3
"""Plot B-02 shared-memory bank-conflict results.

Reads:
  docs/results/B-02_modes.csv

Writes:
  article/02_memory_optim/assets/B-02-mode-ms-bars.png
  article/02_memory_optim/assets/B-02-speedup-vs-naive.png
"""
...
MODES_CSV = ROOT / "docs" / "results" / "B-02_modes.csv"
...
MODE_ORDER = ["naive", "padding", "swizzle"]
MODE_COLOR = {"naive": AMBER, "padding": CYAN, "swizzle": GREEN}
...
def main() -> int:
    ...
    sp = [float(r["speedup_vs_naive"]) for r in rows]
    ...
    ax.set_title("B-02: SMEM column-scan modes (median)")
    for i, v in enumerate(ms):
        ax.text(i, v, f"{v:.3f}", ha="center", va="bottom", color=FG, fontsize=9)
    fig.tight_layout()
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    out1 = OUT_DIR / "B-02-mode-ms-bars.png"
    ...
    ax.set_ylabel("speedup (naive_ms / mode_ms)")
    ax.set_title("B-02: speedup vs naive")
    for i, v in enumerate(sp):
        ax.text(i, v, f"{v:.2f}x", ha="center", va="bottom", color=FG, fontsize=9)
    fig.tight_layout()
    out2 = OUT_DIR / "B-02-speedup-vs-naive.png"
